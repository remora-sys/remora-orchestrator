...
Create a file called `settings.yml` that contains all the configuration parameters for the testbed deployment. You can find an example file in `./assets/settings.yml`.
...
```json
"repository": {
  "url": "http://ghp_ACCESS_TOKEN@github.com/your-org/your-repo.git",
  "commit": "main"
}
```
...
The orchestrator binary provides various functionalities for creating, starting, stopping, and destroying instances.
You can use the following command to boot 5 instances per region (if the settings file specifies 2 regions, as shown in the example above, a total of 10 instances will be created).
In **Remora** (scale-out single-validator execution) experiments, we focus on **single region** by default.
Therefore in the remora experiments, only 5 instances will be created.

```bash
cargo run --bin remora-orchestrator -- testbed deploy --instances 5
```
...
Running benchmarks involves installing the specified version of the codebase on the remote machines and running one validator and one load generator per instance.
For example, the following command benchmarks a committee of 5 instances.
This includes three default nodes: one load generator, one primary node, and one monitor node, while the rest are proxy nodes (2 in this case).
When you increase the committee numbers, proxy nodes will accordingly increase.

```bash
cargo run --bin remora-orchestrator -- benchmark --committee 5
```

Performance measurements are collected by regularly scraping the Prometheus metrics exposed by the load generators.
The `orchestrator` binary provides additional commands to run a specific number of load generators on separate machines.
...
For the naming in the example dashboard (./assets/grafana-dashboard.json), node-0 is for primary node, node-1/2... are for proxy nodes.
